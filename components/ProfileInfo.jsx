...
export function ProfileInfo({ onSave, onBack, initialData = null }) {
  ...
  const validateForm = () => {
    const newErrors = {};
    if (!nameTrimmed) newErrors.name = "이름을 입력해주세요.";

    if (!emailTrimmed) newErrors.email = "이메일을 입력해주세요.";
    else if (!/^([^\s@]+)@([^\s@]+)\.[^\s@]+$/.test(emailTrimmed))
      newErrors.email = "올바른 이메일 형식을 입력해주세요.";

    if (!phoneTrimmed) newErrors.phone = "연락처를 입력해주세요.";
    else if (!/^[0-9\-+\s()]{10,}$/.test(phoneTrimmed.replace(/\s/g, "")))
  ...
  const pickResume = async () => {
    ...
    const okByMime =
      file.mimeType && ALLOWED_RESUME_MIME.includes(file.mimeType);
    const okByExt = ALLOWED_RESUME_EXT.includes(ext);
  ...
  const handleSubmit = async () => {
    ...
    try {
      await new Promise((r) => setTimeout(r, 300)); // 모의 저장
      onSave?.({
        ...formData,
        name: nameTrimmed,
        email: emailTrimmed,
